/**
* Description: This module performs implements matrix multiplication in parallel by forking child processes for each matrix, communicating through pipes, and redirecting output to separate files for each process, ensuring effective inter-process communication and synchronization.
...
int calculateMultiplication(char *inputMatrix, char **matrixList, const int matrixCount) {
    ...
    for (int i = 0; i < matrixCount; ++i) {
        ...
        if (pid < 0) {
            ...
            char *args[] = {"matrixmult_threaded", inputMatrix, matrixList[i], realSOUT, NULL};
            if (execv("./matrixmult_threaded", args) == -1) {
                // Error handling.
                fprintf(stderr, "execv() failed. Command tried to execute: %s %s %s %s\n", "./matrixmult_threaded", args[1], args[2], args[3]);
                close(outFD);
                close(errFD);
                close(pipes[i][0]);
                close(pipes[i][1]);
